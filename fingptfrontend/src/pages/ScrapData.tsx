...
type DataMapState = Record<string, DataResponse | null>;

// ✅ Data sources definition
const dataSources: DataSourceCategory = {
  news: [
    { name: "CCTV News", endpoint: "cctv-news" },
    { name: "Finnhub News", endpoint: "finnhub-news" },
    { name: "AAPL News", endpoint: "aapl-news" },
    { name: "CNBC News", endpoint: "cnbc-news" },
    { name: "Yahoo News", endpoint: "yahoo-news" },
  ],
  socialMedia: [
    { name: "Twitter", endpoint: "twitter" },
    { name: "Reddit", endpoint: "reddit" },
    { name: "LinkedIn", endpoint: "linkedin" },
  ],
  filings: [{ name: "SEC Edgar", endpoint: "sec-edgar" }],
  datasets: [
    { name: "Yahoo Finance", endpoint: "yahoo-news" },
    { name: "Alpha Vantage", endpoint: "alpha-vantage" },
  ],
};
...
export function ScrapData() {
  ...
  const [selectedCategory, setSelectedCategory] = useState<string | null>(null);
  const [expandedSources, setExpandedSources] = useState<ExpandedState>({});
  const [dataMap, setDataMap] = useState<DataMapState>({});
  ...
  const handleScrape = async (endpoint: string, sourceName: string) => {
    setLoading((prev) => ({ ...prev, [sourceName]: true }));
    setError((prev) => ({ ...prev, [sourceName]: null }));
    ...
    try {
      ...
      if (response.ok) {
        setDataMap((prev) => ({ ...prev, [sourceName]: data }));
      } else {
        setError((prev) => ({
          ...prev,
          [sourceName]: data.error || "Error fetching data",
        }));
      }
    } catch (err) {
      setError((prev) => ({
        ...prev,
        [sourceName]: "Server not reachable",
      }));
    } finally {
      setLoading((prev) => ({ ...prev, [sourceName]: false }));
      setExpandedSources((prev) => ({ ...prev, [sourceName]: true }));
    }
  };
  ...
  return (
    <div
      className={`w-full min-h-screen flex transition-colors duration-300 ${
        isDarkMode
          ? "bg-gray-900"
          : "bg-gradient-to-br from-[#FAF8F5] via-[#F0F4F8] to-[#E8F5F3]"
      }`}
    >
      <Sidebar isDarkMode={isDarkMode} setIsDarkMode={setIsDarkMode} />
      <div className="flex-1 ml-64 h-screen overflow-y-auto p-6">
          ...
          {!selectedCategory ? (
            <div className="grid md:grid-cols-2 gap-4">
              {categories.map((category) => (
                <motion.button
                  ...
                  className="bg-white/60 backdrop-blur-sm p-6 rounded-xl border border-white/20 hover:bg-white/70 transition-all text-left"
                >
                  <div className="text-4xl mb-2">{category.icon}</div>
                  <h3 className="text-lg font-bold text-gray-800">
                    {category.label}
            ...
            <div>
              <button
                ...
                className="mb-4 px-4 py-2 rounded-lg bg-white/60 backdrop-blur-sm text-gray-800 hover:bg-white/70 transition-all border border-white/20 text-sm font-medium"
              >
                ← Back
              </button>

              <div className="space-y-3">
                {dataSources[selectedCategory]?.map(({ name, endpoint }) => (
                  <div
                    key={name}
                  ...
                  >
                    <div className="p-4 flex items-center justify-between">
                      ...
                      <button
                        onClick={() => handleScrape(endpoint, name)}
                        disabled={loading[name]}
                        className="flex items-center gap-2 px-4 py-1.5 bg-gradient-to-r from-teal-500 to-teal-600 text-white rounded-lg hover:shadow-md transition-all text-sm font-medium"
                      >
                        {loading[name] ? (
                          <>
                          ...
                          <div className="p-4">
                            {error[name] ? (
                              ...
                              <>
                                <div className="mb-3 p-3 rounded-lg bg-teal-50/50 border border-teal-100">
                                  <div className="flex items-center justify-between mb-2">
                                    ...
                                    <button
                                      onClick={() =>
                                        navigate(
                                          `/data-details?source=${encodeURIComponent(
                                            name
                                  ...
                                  <ul className="space-y-0.5 text-xs text-gray-600">
                                    <li>
                                      •{" "}
                                      {dataMap[name]?.message ||
                                        "Data fetched successfully"}
                                    </li>
                                    <li>
                                      • {dataMap[name]?.rows ?? 0} records found
                                    </li>
                                  </ul>
                                </div>
                                ...
                                <div className="overflow-x-auto">
                                  <table className="w-full text-sm">
                                    <thead className="bg-gray-50/50 rounded-lg">
                                      <tr>
                                        {dataMap[name]?.preview &&
                                          Object.keys(
                                            dataMap[name]?.preview[0] || {}
                                          ).map((key) => (
                                            <th
                                              key={key}
                                              className="p-2 text-left text-gray-800 font-semibold capitalize"
                                            >
                                              {key}
                                            </th>
                                          ))}
                              ...
                              <p className="text-sm text-gray-500">
                                Click “Scrape Data” to fetch latest data.
                              </p>
                            )}
                          </div>
                        </motion.div>
