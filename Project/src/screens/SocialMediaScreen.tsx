...
const SocialMediaScreen: React.FC = () => {
  ...
  const [socialMediaSources, setSocialMediaSources] = useState<SocialMediaSource[]>([]);
  ...
  useEffect(() => {
    loadSocialMediaSources();
  }, []);

  const loadSocialMediaSources = async () => {
    try {
      setLoading(true);
      const response = await apiService.getDataSources('socialMedia');
      if (response.success) {
        setSocialMediaSources(response.data);
      } else {
        Alert.alert('Error', 'Failed to load social media sources');
      }
    } catch (error) {
      Alert.alert('Error', 'Failed to load social media sources');
    } finally {
  ...
  const handleScrape = async (source: string) => {
    try {
      ...
      const response = await apiService.scrapeData(source, 'socialMedia');
